...
import { getPublicSiteSettings } from "@/lib/content/loaders";

export async function SiteFooter() {
  const { socialLinks, contactInfo } = await getPublicSiteSettings();
  ...
  const socialItems = [
    ...
    socialLinks.x || socialLinks.twitter
      ? { href: (socialLinks.x || socialLinks.twitter) as string, label: "X" }
      : null,
    ...
    <footer className="border-t border-slate-200 bg-navy-950 text-slate-200">
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-3 lg:px-8">
        <div>
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-300">{COMPANY.legalName}</p>
          <p className="mt-4 text-sm text-slate-400">CIN: {COMPANY.cin}</p>
              ...
              {socialItems.map((item) => (
                <li key={item.href}>
                  <a
                    ...
                    className="inline-flex items-center gap-2 rounded-md border border-white/15 px-3 py-1.5 text-sm text-slate-200 hover:border-teal-300 hover:text-teal-300"
                  >
                    {item.label}
                  </a>
                </li>
        ...
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-slate-400">Explore</p>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            <li>
              <a className="hover:text-teal-300" href={`mailto:${email}`}>
                {email}
              </a>
            </li>
            <li>
              <a className="hover:text-teal-300" href={`tel:${telephone.replace(/\s/g, "")}`}>
                {telephone}
              </a>
            </li>
            <li className="pt-2 text-slate-400">Queries / Grievances: {COMPANY.grievances}</li>
          </ul>
          <p className="mt-6 text-sm font-semibold uppercase tracking-wider text-slate-400">Legal</p>
